...
node {
    ...
    docker.image('jhipster/jhipster:v5.7.2').inside('-u root -e MAVEN_OPTS="-Duser.home=./"') {
        ...
        stage('package and deploy') {
            sh "./mvnw com.heroku.sdk:heroku-maven-plugin:2.0.5:deploy -DskipTests -Pprod -Dheroku.buildpacks=heroku/jvm -Dheroku.appName=tuto-j-hipster"
    ...
    stage('build docker') {
        ...
        dockerImage = docker.build('docker-login/tutojhipster', 'target/docker')
    }
